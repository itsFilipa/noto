...
interface TagStore {
  ...
  actions: {
    ...
    getTagByUserId: (
      userId: string
    ) => Promise<{ tags: Tag[] | null; error: Error | null }>;
    getTag: (id: string) => Promise<{ tag: Tag | null; error: Error | null }>;
    createTag: (
      payload: string
    ) => Promise<{ tag: Tag | null; error: Error | null }>;
  };
}
...
export const useTags = () => {
  ...
  const { getTagByUserId, getTag, createTag, setLoading, setTags } =
    useTagStore((state) => state.actions);
